 ...
 *
 * 选择器提供程序是此类的具体子类，具有零参数构造函数并实现下面指定的抽象方法。给定的Java虚拟机调用维护单个
 * 系统范围的默认提供程序实例，由{@link #provider()}方法。第一次调用该方法将找到下面指定的默认提供程序。
 *
 ...
 * threads.  </p>
 *
 * 系统范围的默认提供程序由 {@link DatagramChannel#open}，{@link Pipe＃open}，{@link Selector#open}，
 * {@link ServerSocketChannel#open}和{@link SocketChannel#open}类。
 * 它也被{@link System#inheritedChannel()} 使用方法。程序可以通过实例化该提供程序然后直接调用此类中定义的open方法来使用默认提供程序以外的提供程序。
 * 此类中的所有方法都可以安全地由多个并发使用
 * 线程。
 *
 *
