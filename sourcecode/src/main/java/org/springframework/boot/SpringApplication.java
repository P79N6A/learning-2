...
public class SpringApplication {
	...
	public SpringApplication(ResourceLoader resourceLoader, Class<?>... primarySources) {
		...
		// 设置application context，默认6个实现
		setInitializers((Collection) getSpringFactoriesInstances(
				ApplicationContextInitializer.class));

		// 设置application listener，默认11个实现
		setListeners((Collection) getSpringFactoriesInstances(ApplicationListener.class));

		// 获取main application class
	...
	private void prepareContext(ConfigurableApplicationContext context,
		...
		// 添加两个Spring Boot中的特殊单例Beans，SpringApplicationArguments和SpringBootBanner
	 ...
	 *
	 * 将设置将在ApplicationContext中使用的ClassLoader(如果设置了{@link #setResourceLoader(ResourceLoader) resourceLoader}，
	 * 或者上下文类加载器(如果不为null)，或Spring {@link ClassUtils}类的加载器。
	 *
	...
	protected BeanDefinitionLoader createBeanDefinitionLoader(
		 ...
		 * 作为{@link AnnotatedBeanDefinitionReader}，{@ link XmlBeanDefinitionReader}和{@link ClassPathBeanDefinitionScanner}的简单外观。
		 * 有关支持的源类型，请参阅{@link SpringApplication}。
		 *
		 * 此类会与spring的BeanDefinitionReader进行关联，从而利用spring加载bean资源
		 */
	 ...
	 *
	 * 设置应用程序是否无头，不应实例化AWT。默认为{@code true}以防止出现java图标。
	 *
	 ...
	 *
	 * 静态助手，可用于使用默认设置从指定源运行{@link SpringApplication}。
	 *
