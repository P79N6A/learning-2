...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    // VisibleForTesting。此标志用于了解qLearner和qServer的登录上下文是否已初始化，因为ApacheDS存在并发问题。
    // 请参阅https://issues.apache.org/jira/browse/ZOOKEEPER-2712
     ...
     *
     * ZKDatabase是quorumpeer的顶级成员，将在稍后实例化的所有zookeeper servers中使用。
     * 此外，它在启动时创建一次，并且只有在来自领导者的截断消息的情况下才会被丢弃
     */
    ...
    public static class QuorumServer {
         ...
         *
         * 执行主机名的DNS查找，并(重新)创建this.addr和this.electionAddr InetSocketAddress对象
         * <P>
         * 如果DNS查找失败，则this.addr和electionAddr保持不变，除非它们从未设置过。 如果this.addr为null，
         * 则使用未解析的InetSocketAddress对象进行设置。 this.electionAddr的处理方式类似。
         */
