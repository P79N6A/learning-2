 ...
 * <p>
 * 设置一个简单线程的类，它在循环中运行一小段时间。如果睡眠时间明显长于其目标时间，
 * 则意味着JVM或主机已暂停处理，这可能会导致其他问题。如果检测到这样的暂停，则线程记录消息。
 */
...
public class JvmPauseMonitor {
    ...
    /**
     * 默认10s, log WARN if we detect a pause longer than this threshold
     */
    ...
    /**
     * 默认1s, log INFO if we detect a pause longer than this threshold
     */
    ...
    private class Monitor implements Runnable {
        ...
        public void run() {
            ...
            while (shouldRun) {
                ...
                if (extraSleepTime > warnThresholdMs) { // elapsed time > 10s
                    ++numGcWarnThresholdExceeded;
                    LOG.warn(formatMessage(
                            extraSleepTime, gcTimesAfterSleep, gcTimesBeforeSleep));
                } else if (extraSleepTime > infoThresholdMs) {  // elapsed time > 1s
