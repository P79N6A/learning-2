...
public class ShutdownHook {
     ...
     * <p>
     * 安装一个关闭钩子，在传递的Stoppable上调用stop，然后对传递的<code>threadToJoin</code>进行线程连接。
     * 当这个线程完成时，它然后运行hdfs线程(这个安装删除了hdfs shutdown钩子，在<code>threadToJoin</code>停止后保持它的句柄运行它)。
     * <p>
     * 要禁止所有关闭挂钩处理 - 运行regionserver挂钩和hdfs挂钩代码 -
     * 将{@link Configuration}中的{@link ShutdownHook＃RUN_SHUTDOWN_HOOK}设置为<code>false</code>。
     * 挂钩代码运行时会检查此配置值。
     *
     * @param conf
