...
public class KeyedStream<T, KEY> extends DataStream<T> {
	 ...
	 * set using
	 * {@link org.apache.flink.streaming.api.environment.StreamExecutionEnvironment#setStreamTimeCharacteristic(TimeCharacteristic)}
	 *
	 * Windows {@code KeyedStream}进入翻滚时间窗口。
	 * 这是{@code .window(TumblingEventTimeWindows.of(size))}或{@code .window(TumblingProcessingTimeWindows.of(size))}的快捷方式，
	 * 具体取决于使用{@link org.apache.flink.streaming.api.environment.StreamExecutionEnvironment＃setStreamTimeCharacteristic(TimeCharacteristic)}
	 *
	 ...
	 * {@link org.apache.flink.streaming.api.environment.StreamExecutionEnvironment#setStreamTimeCharacteristic(TimeCharacteristic)}
	 *
	 * Windows {@code KeyedStream}进入滑动时间窗口。
	 *
	 * 这是{@code .window(SlidingEventTimeWindows.of(size，slide))}或{@code .window(SlidingProcessingTimeWindows.of(size, slide))}的快捷方式，
	 * 具体取决于使用{@link org.apache.flink.streaming.api.environment.StreamExecutionEnvironment＃setStreamTimeCharacteristic(TimeCharacteristic)}
	 *
	 * @param size The size of the window.
	 */
	 ...
	 *
	 * Windows {@code KeyedStream}进入翻滚计数窗口
	 *
	 ...
	 *
	 * Windows {@code KeyedStream}进入滑动计数窗口
	 *
	 * @param size The size of the windows in number of elements.
	 ...
	 *
	 * 将此数据流Windows转换为{@code WindowedStream}，该窗口通过key分组流评估窗口。元素由{@link WindowAssigner}放入窗口。元素分组由键和窗口完成。
	 *
	 * 可以定义{@link org.apache.flink.streaming.api.windowing.triggers.Trigger}来指定何时评估窗口。
	 * 但是，{@ code WindowAssigners}具有默认的{@code Trigger}，如果未指定{@code Trigger}，则使用该默认值。
	 *
