...
public class DataStream<T> {
	 ...
	 * its operator states.
	 *
	 * 创建了一个新的{@link KeyedStream}，使用提供的key对其运算符状态进行分区
	 *
	 * @param key
	 ...
	 *
	 * 创建了一个新的{@link KeyedStream}，使用提供的key和显式类型信息来分区其运算符状态
	 *
	 ...
	 *
	 * 按给定的键位置对{@link DataStream}的运算符状态进行分区
	 *
	 * @param fields
	 ...
	 *
	 * 使用字段表达式对{@link DataStream}的运算符状态进行分区。字段表达式可以是公共字段的名称，
	 * 也可以是带有{@link DataStream}基础类型括号的getter方法。可以使用点来深入到对象中，如{@code“field1.getInnerField2()”}中所示。
	 *
	 * @param fields
	 ...
	 *
	 * {@link StreamExecutionEnvironment#setStreamTimeCharacteristic(TimeCharacteristic)}
	 *
	 ...
	 * {@link WindowAssigner}. The grouping of
	 ...
	 * method is called.
	 *
	 * 将给定的接收器添加到此DataStream。只有在调用{@link StreamExecutionEnvironment＃execute()}方法后，才会执行添加了带有接收器的流。
	 *
	 * @param sinkFunction
