 ...
 * <p>
 * StreamExecutionEnvironment是执行流程序的上下文。
 * {@link LocalStreamEnvironment}将导致在当前JVM中执行，{@link RemoteStreamEnvironment}将导致在远程设置上执行。
 * <p>
 * 环境提供了控制作业执行的方法(例如设置并行性或容错/检查点参数)以及与外部世界交互(数据访问)
 *
...
public abstract class StreamExecutionEnvironment {
     ...
     *
     * 设置通过此环境执行的操作的并行度。在此处设置x的并行性将导致所有运算符(例如map，batchReduce)与x个并行实例一起运行。
     * 此方法将覆盖此环境的默认并行度。{@link LocalStreamEnvironment}默认使用的值等于硬件上下文的数量(CPU核心/线程)。
     * 通过命令行客户端从JAR文件执行程序时，默认的并行度是为该设置配置的并行度。
     *
     ...
     *
     * 设置为程序定义的最大并行度。上限(包括)为Short.MAX_VALUE。
     * 最大并行度指定动态缩放的上限。它还定义了用于分区状态的密钥组的数量
     *
     ...
     * parallelism.
     *
     * 获取默认执行操作的并行度。操作可以单独覆盖此值以使用特定的并行性。
     *
     ...
     * @param watchType      The watch type of file stream. When watchType is {@link FileMonitoringFunction.WatchType#ONLY_NEW_FILES}, the system processes
     *                       only
     ...
     * decoded by the system's default character set. The reader is terminated immediately when the socket is down.
     *
     * 创建一个新数据流，其中包含从套接字无限接收的字符串。接收的字符串由系统的默认字符集解码。当socket关闭时，reader立即终止
     *
     * @param hostname  The host name which a server socket binds
     * @param port      The port number which a server socket binds. A port number of 0 means that the port number is automatically
     *                  allocated.
     ...
     *
     * 将数据源添加到流式拓扑
     * 默认情况下，源的并行性为1。要启用并行执行，用户定义的源应实现{@link ParallelSourceFunction}
     * 或扩展{@link org.apache.flink.streaming.api.functions.source.RichParallelSourceFunction}。
     * 在这些情况下，生成的源将具有环境的并行性。要改变它，然后调用{@link DataStreamSource#setParallelism(int)}
     *
     ...
     * 只有在非常特殊的情况下，用户才需要支持类型信息。否则使用{@link #addSource(SourceFunction)}
     *
     * @param function   the user defined function
     ...
     *
     * 创建一个执行环境，表示当前执行程序的上下文。如果程序是独立调用的，
     * 则此方法返回由{@link #createLocalEnvironment()}返回的本地执行环境
     *
