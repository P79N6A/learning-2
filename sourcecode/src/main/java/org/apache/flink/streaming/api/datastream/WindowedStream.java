...
public class WindowedStream<T, K, W extends Window> {
	 ...
	 *
	 * 设置允许元素延迟的时间。到达水印后超过指定时间的元素将被删除。默认情况下，允许的延迟是{@code 0L}
	 *
	 * 设置允许的延迟仅对事件时间窗口有效
	 */
	@PublicEvolving
	 ...
	 *
	 * 将给定的窗口函数应用于每个窗口。为每个键的窗口的每个评估单独调用窗口函数。窗口函数的输出被解释为常规的非窗口流。
	 * 并不是说这个函数要求窗口中的所有数据都被缓冲，直到评估窗口，因为该函数不提供增量聚合的方法。
	 *
	 * @param function The window function.
	 * @return The data stream that is the result of applying the window function to the window.
	 */
