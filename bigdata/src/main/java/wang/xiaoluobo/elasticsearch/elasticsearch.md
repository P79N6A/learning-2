...
1. 修改 linux 内核参数
    ...
    [root@hadoop01 ~]# vim /etc/security/limits.d/90-nproc.conf
    *          soft    nproc     2048
    ```
...
2. 修改 es 配置文件
    ```bash
    # 创建 es 数据和 log 目录
    [hadoop@hadoop01 elasticsearch-5.3.0]$ mkdir -p /mnt/data/elastic /mnt/log/elastic
    [hadoop@hadoop01 elasticsearch-5.3.0]$ vim config/elasticsearch.yml
    ```

3. 启动 es 集群
    ```bash
    [hadoop@hadoop01 elasticsearch-5.3.0]$ nohup ./bin/elasticsearch &
    [hadoop@hadoop02 elasticsearch-5.3.0]$ nohup ./bin/elasticsearch &
    [hadoop@hadoop03 elasticsearch-5.3.0]$ nohup ./bin/elasticsearch &
    ```

4. 验证 es 集群
    ```text
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl http://172.16.18.13:9200
    {
      "name" : "node-1",
    ...
    [hadoop@hadoop02 elasticsearch-5.3.0]$ curl http://172.16.18.14:9200
    {
      "name" : "node-2",
      "cluster_name" : "es_cluster",
      "cluster_uuid" : "aGi9qM1DSaqmhE0IHVLfOA",
    ...
    [hadoop@hadoop03 elasticsearch-5.3.0]$ curl http://172.16.18.15:9200
    {
      "name" : "node-3",
      "cluster_name" : "es_cluster",
      "cluster_uuid" : "aGi9qM1DSaqmhE0IHVLfOA",
    ...
    # 查看集群状态
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl http://172.16.18.13:9200/_cat/health?v
...
1. 创建索引
    ...
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl -XPUT http://172.16.18.13:9200/test -d '
    > {
    >     "settings" : {
    >         "number_of_shards" : 1
    >     },
...
2. 查看集群的健康状态
    ...
    epoch      timestamp cluster    status node.total node.data shards pri relo init unassign pending_tasks max_task_wait_time active_shards_percent
    1551431015 17:03:35  es_cluster green           3         3      0   0    0    0        0             0                  -                100.0%
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl -XGET http://172.16.18.13:9200/_cluster/health?pretty
    {
      "cluster_name" : "es_cluster",
      "status" : "green",
      "timed_out" : false,
    ...
    [hadoop@hadoop02 elasticsearch-5.3.0]$ curl http://172.16.18.14:9200/_cat/health?v
    epoch      timestamp cluster    status node.total node.data shards pri relo init unassign pending_tasks max_task_wait_time active_shards_percent
    1551431046 17:04:06  es_cluster green           3         3      0   0    0    0        0             0                  -                100.0%
    
    [hadoop@hadoop03 elasticsearch-5.3.0]$ curl http://172.16.18.15:9200/_cat/health?v
    epoch      timestamp cluster    status node.total node.data shards pri relo init unassign pending_tasks max_task_wait_time active_shards_percent
...
3. 查看集群的索引数
    ```bash
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl http://172.16.18.13:9200/_cat/indices?v
    health status index uuid pri rep docs.count docs.deleted store.size pri.store.size
    ```

4. 查看集群所在磁盘的分配状况
    ```bash
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl http://172.16.18.13:9200/_cat/allocation?v
...
5. 查看集群目录信息
    ```text
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl http://172.16.18.13:9200/_cat/
    =^.^=
    /_cat/allocation
    /_cat/shards
    /_cat/shards/{index}
...
6. 查询主节点信息
    ```bash
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl -XGET http://172.16.18.13:9200/_cat/master?v
    id                     host         ip           node
...
7. 查看节点信息
    ```bash
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl -XGET http://172.16.18.13:9200/_cat/shards?v
...
8. 查询集群状态
    ```text
    [hadoop@hadoop01 elasticsearch-5.3.0]$ curl -XGET 172.16.18.13:9200/_cluster/stats?pretty=true
    {
      "_nodes" : {
        "total" : 3,
        "successful" : 3,
